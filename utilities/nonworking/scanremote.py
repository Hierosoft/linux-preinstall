...
if config.get("scanner_name") is None:
    ...
    for scanner_line in scanner_lines:
        ...
        if config.get("scanner_name") is None:
            ...
            print('* additional scanner (ignored unless specified with'
                  ' --scanner_name ): {}'.format(parts2[0]))
...
try:
    ...
    rm_output_bytes = subprocess.check_output(
...
save_config()
print("* scanning...")
# remote_scan_cmd = ('scanimage -d "{}" --format=tiff | convert tiff:- {}.jpg'
#                    ''.format(config["scanner_name"], remote_scanned_path))
remote_scan_cmd = ('scanimage -d "{}" --format=jpeg > {}.jpg'
                   ''.format(config["scanner_name"], remote_scanned_path))
print("  " + remote_scan_cmd)
'''
such as: scanimage -d "brother4:bus8;dev1" --format=jpeg --source FlatBed
such as: scanimage -d "brother4:bus8;dev1" --format=tiff | \
             convert tiff:- FlowChart-1a-handmade.jpg
  Geometry:
    ...
    -y 0..355.6mm (in steps of 0.0999908) [355.567]
        Height of scan-area.
'''
scan_output_bytes = subprocess.check_output(
    ['ssh', '{}@{}'.format(config["remote_user"],
     config["remote_host"]), "'{}'".format(remote_scan_cmd)]
)
scan_output = scan_output_bytes.decode('utf-8').strip()
print(scan_output)
print('* transferring file from "{}" to "{}"...'
      ''.format(config["remote_host"], client_remote_path))
remote_transfer_cmd = ('rsync -tv "{}" "{}"'
                       ''.format(remote_scanned_path, client_remote_path))
print("  " + remote_transfer_cmd)
# such as: rsync -t FlowChart-1a-handmade.jpg   pgs:/home/owner
transfer_output_bytes = subprocess.check_output(
    ['ssh', '{}@{}'.format(config["remote_user"],
     config["remote_host"]), "'{}'".format(remote_transfer_cmd)]
)
...
if rotate != 0:
    ...
    if os.path.isfile(output):  # /usr/bin/jpegtran
        ...
        rotate_cmd = ('jpegtran -rotate {} -outfile {} {}'
                      ''.format(rotate, out_path, scanned_path))
        ...
        if os.path.isfile(out_path):
            ...
            print(rotate_cmd)
            print("ERROR: rotate command above did not result in '{}'"
                  "".format(out_path))
            save_config()
            exit(1)
    else:
        print("To use jpegtran for lossless rotation,"
              " you must first install libjpeg-turbo-utils.")
