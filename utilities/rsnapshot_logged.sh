...
if [ ! -f "$RESULT_RC" ]; then
    ...
    cat "$OUTFILE"
    cat "$LOG"
    echo "Error: missing $RESULT_RC" | tee -a $LOG
    exit 1
else
    echo "Running '$RESULT_RC'"
fi
. $RESULT_RC
if [ -z "$snapshot_root" ]; then
    echo "Error: no snapshot_root in $RESULT_RC" | tee -a $LOG
    exit 2
fi

if [ -z "$backup_drive" ]; then
    echo "Error: no backup_drive in $RESULT_RC" | tee -a $LOG
    exit 3
fi

if [ -z "$dst_logs" ]; then
    echo "Error: no dst_logs in $RESULT_RC" | tee -a $LOG
    exit 3
fi

if [ ! -d "$dst_logs" ]; then
    echo "Error: rsnapshot_logged.py did not create the dst_logs folder \"$dst_logs\"." | tee -a $LOG
    exit 3
fi
