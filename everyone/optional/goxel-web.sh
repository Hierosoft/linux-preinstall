#!/bin/bash
_WEB_INSTALL_URL=https://raw.githubusercontent.com/Hierosoft/linux-preinstall/master/install-via-web.sh
_GOOD_FLAG_FILE=~/Downloads/git/Hierosoft/linux-preinstall/install-via-web.sh
...
_GOXEL_DEPS_SCRIPT=~/Downloads/git/Hierosoft/linux-preinstall/everyone/optional/goxel.sh
...
if [ $? -eq 0 ]; then
    $HOME/Downloads/git/Hierosoft/linux-preinstall/everyone/optional/goxel-nonroot.sh
    if [ $? -eq 0 ]; then
        if [ -f "$_PROGRAM_SRC_BIN_FILE_PATH" ]; then
            ...
            echo "$HOME/Downloads/git/Hierosoft/linux-preinstall/everyone/optional/goxel-nonroot.sh did not result in."
            exit 1
        fi
    else
        echo "$HOME/Downloads/git/Hierosoft/linux-preinstall/everyone/optional/goxel-nonroot.sh failed."
        exit 1
    fi
else
